...
class DeviceManage(CoreModel):
    ...
    production_status = models.IntegerField(choices=CHOICES_LIST, default=0, help_text="生产状态", verbose_name="生产状态")
    TYPE_LIST = (
        (0, "码包管理端"),
        (1, "检测管理端")
    )
    type = models.IntegerField(choices=TYPE_LIST, default=0, help_text="设备类型", verbose_name="设备类型")
    user = models.ForeignKey(Users, db_constraint=False, related_name="user_device", on_delete=models.CASCADE, blank=True,
...
class CodePackageTemplate(CoreModel):
    ...
    code_source = models.IntegerField(choices=CODE_SOURCE,default=1,blank=True,help_text="编码来源",verbose_name="编码来源")
