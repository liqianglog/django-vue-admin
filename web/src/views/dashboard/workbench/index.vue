...
export default {
  ...
  methods: {
    ...
    initLineChart () {
      ...
      this.global.homeChartOne = echarts.init(this.$refs.homeLineRef, this.charts.theme)
      const option = {
        backgroundColor: this.charts.bgColor,
        title: {
          text: '政策补贴额度',
          x: 'left',
          textStyle: { fontSize: '15', color: this.charts.color }
        },
      ...
      this.myCharts.push(this.global.homeChartOne)
    },
    // 饼图
    initPieChart () {
      if (!this.global.dispose.some((b) => b === this.global.homeChartTwo)) this.global.homeChartTwo.dispose()
      this.global.homeChartTwo = echarts.init(this.$refs.homePieRef, this.charts.theme)
      ...
      this.myCharts.push(this.global.homeChartTwo)
    },
    // 柱状图
    initBarChart () {
      if (!this.global.dispose.some((b) => b === this.global.homeCharThree)) this.global.homeCharThree.dispose()
      this.global.homeCharThree = echarts.init(this.$refs.homeBarRef, this.charts.theme)
      const option = {
        backgroundColor: this.charts.bgColor,
        title: {
          text: '地热开发利用',
        ...
        yAxis: [
          {
            ...
            axisLine: { show: false },
            axisTick: { show: false },
            axisLabel: { color: this.charts.color, formatter: '{value} ' }
          },
          {
            name: '压力值(Mpa)',
            nameLocation: 'middle',
      ...
      this.myCharts.push(this.global.homeCharThree)
    },
    // 批量设置 echarts resize
    ...
    // 批量设置 echarts resize
    initEchartsResize () {
      window.addEventListener('resize', this.initEchartsResizeFun())
    }
  },
  mounted () {
